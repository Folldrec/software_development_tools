 ...
 * list.set(500, 99);
 * 
 * // Memory usage is minimal - only 2 values stored
...
private:
    std::map<size_t, T> data;      ///< Map storing index -> value pairs
    size_t listSize;                ///< Total size of the list
    T defaultValue;                 ///< Default value for unset elements
...
public:
    ...
    int findByValue(const T& value) const override {
        if (value == defaultValue) {
            // Шукаємо перший індекс, якого немає в data
